# Product Repair & Publish Pipeline

This repository contains a 4-step repair and publishing pipeline for repairing product/variant linkages on the product side, handling archived products safely, and republishing affected variants.

It mirrors the variant-side pipeline — same architecture (scan → PUT → batched COMMIT), same multi-site contract, same no-READY publish flow — but operates on `/pim/products/` endpoints and the inverse direction of the linkage (product's `variantIds` rather than variant's `productIds`).
...
INPUT: report-linkages.xlsx
        │
        ▼
1. repair_products.py
        │
        ├─ product_map.json
        ├─ duplicate_variants_across_product.json
        ├─ variant_states_pre_repair.json
...
2. push_products.py
        │
        ▼
3. repair_archived_products.py
        │
        ▼
4. publish_pending_variants.py
```

---

## Script 1 — `repair_products.py`
...
Each row contributes a product entry. The variant-list column is sheet-specific:
...
issue_label   = "PARENT_DOES_NOT_LINK_BACK"
variants_cell = "BA-XYZ123 (Foo), BA-ABC456 (Bar)"

# Format B — code embedded as prefix
variants_cell = "PARENT_DOES_NOT_LINK_BACK: BA-XYZ123 (Foo), BA-ABC456 (Bar)"
```
...
are retained. Other codes (`STALE_LATEST_EMPTY`, `ORPHAN_BUT_REFERENCED`, etc.) are handled by other scripts and silently dropped.

#### 2. Merge + Detect Duplicates

Products are merged across sheets — the same `productId` appearing in multiple sheets has its variantIds combined into one deduplicated list, with the contributing sheets tracked in `sources`.
...
The enriched data rewrites `duplicate_variants_across_product.json`.
...
This is the inverse of the variant-side script's product-states snapshot: that script captured product states; this one captures **variant** states. It's the authoritative "before" snapshot used by `publish_pending_variants.py` (script 4) to decide which variants need republishing after products get updated.
...
Original payload order is preserved; new IDs not already present are appended. This is the smallest possible change that makes the linkage right.
...
After repair completes, `product_map.json` is rewritten using authoritative API values:
...
Looks up metadata in `product_map.json`.
...
Skipped because archived products are handled separately by `repair_archived_products.py`.
...
## Script 3 — `repair_archived_products.py`
...
## Script 4 — `publish_pending_variants.py`

Publishes variants that received new drafts during product repair.
...
Loads `variant_states_pre_repair.json`.
...
|---|---|
| `product_map.json` | Product metadata map with authoritative API state |
| `duplicate_variants_across_product.json` | Variants claimed by 2+ different products |
| `variant_states_pre_repair.json` | Variant states (state, status) before repair |
...
python3 push_products.py             --input ./out-products --dry-run
...
# Real runs
python3 push_products.py             --input ./out-products --workers 4
python3 repair_archived_products.py  --input ./out-products --workers 4
python3 publish_pending_variants.py  --input ./out-products --workers 4
```
...
- Archived products are never touched by `push_products.py` (handed off to script 3)
