...
Final script in the NEGATIVE product-repair toolchain. Runs AFTER:
    repair_products_negative.py             (produces
                                             variant_states_pre_repair_negative.json)
    push_products_negative.py               (PUTs products — may create
                                             variant drafts)
    repair_archived_products_negative.py    (the archived-product flow)
...
that gets re-linked. This is what "auto-generated" refers to: variants
the platform spun up drafts for as a side-effect of the product PUTs,
NOT drafts the operator created by hand. If the variant was PUBLISHED
before the run started, that auto-generated draft needs to be promoted
to live so the new product↔variant linkage actually goes out. This
script finds those variants and publishes them.

The two pipelines (positive and negative) each have their own snapshot
file, so this script's filter only sees the variants that the negative
pipeline touched — the positive pipeline's variants stay out of scope.

Filter
------
From variant_states_pre_repair_negative.json, take only variants where
    state == "PUBLISHED"
...
created (auto-generated) draft promoted.

The "status" field in variant_states_pre_repair_negative.json was captured
at the start of repair_products_negative.py's run — so it's the
authoritative "before" state we use for this decision.

Usage
-----
    # Dry run — see counts, no HTTP traffic
    python3 publish_auto_generated_variants_negative.py \\
        --input ./out-products-negative --dry-run

    # Check phase only — probe which variants have drafts, no COMMIT
    python3 publish_auto_generated_variants_negative.py \\
        --input ./out-products-negative --stop-after check

    # Full run
    export API_COOKIE='...'
    python3 publish_auto_generated_variants_negative.py \\
        --input ./out-products-negative --workers 8
"""
...
def setup_logging(log_dir: Path, verbose: bool = False) -> logging.Logger:
    ...
    Three rotating handlers + console mirror at INFO+:
      ...
      publish-auto-generated-variants-negative.run.log      — everything (DEBUG if --verbose, else INFO)
      publish-auto-generated-variants-negative.success.log  — INFO-level per-variant verdicts
      publish-auto-generated-variants-negative.errors.log   — WARNING+ (propagates to run.log)
    """
    ...
    main.addHandler(_rotating(
        log_dir / "publish-auto-generated-variants-negative.run.log",
    ...
    ok.propagate = False
    ok.handlers.clear()
    ok.addHandler(_rotating(log_dir / "publish-auto-generated-variants-negative.success.log", logging.INFO))

    err = logging.getLogger("variants.err")
    err.setLevel(logging.WARNING)
    err.propagate = True
    err.handlers.clear()
    err.addHandler(_rotating(log_dir / "publish-auto-generated-variants-negative.errors.log", logging.WARNING))
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="Output dir from repair_products_negative.py "
             "(contains variant_states_pre_repair_negative.json)",
    )
    ap.add_argument(
        "--api-host",
    ...
    ap.add_argument(
        ...
        help="Concurrent check + batch workers (default 8)",
    )
    ap.add_argument(
        "--batch-size", type=int, default=50,
        help="Max variants per COMMIT batch (default 50)",
    )
    ap.add_argument(
    ...
    vstate_path = args.input / "variant_states_pre_repair_negative.json"
    if not vstate_path.exists():
        log.error("variant_states_pre_repair_negative.json not found at %s", vstate_path)
        return 1
    variant_states = json.loads(vstate_path.read_text())
    log.info("loaded variant_states_pre_repair_negative.json with %d variants", len(variant_states))
