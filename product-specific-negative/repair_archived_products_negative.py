...
Negative-pipeline counterpart of repair_archived_products.py.

Same script, same flow — but reads `product_map_negative.json` (the
output of repair_products_negative.py) instead of the positive map. The
two pipelines stay in their own lanes: the positive one handles
PARENT_DOES_NOT_LINK_BACK products, this one handles
PRODUCT_DROPPED_VARIANT products. Independent input files mean both can
run from the same workspace without cross-contamination.

What this script does (identical to repair_archived_products.py)
----------------------------------------------------------------

An archived product can't be updated directly — the platform forbids
writes to archived. To repair an archived product whose payload has been
modified, we have to:
...
That's two round-trip state changes plus the actual PUT.
push_products_negative.py deliberately avoids touching archived products
so its happy-path stays simple; this script owns the more involved flow.
...
Filter
------
Only processes products in product_map_negative.json where:
...
-----
    ...
    python3 repair_archived_products_negative.py --input ./out-products-negative --stop-after recover
"""
...
# ---------------------------------------------------------------------------
# Site configuration — same as repair_products_negative.py / push_products_negative.py
# ---------------------------------------------------------------------------
...
def setup_logging(log_dir: Path, verbose: bool = False) -> logging.Logger:
    ...
    Three rotating handlers + console mirror at INFO+:
      ...
      archived-products-negative.run.log      — everything (DEBUG if --verbose else INFO)
      archived-products-negative.success.log  — per-product verdicts (INFO only)
      archived-products-negative.errors.log   — WARNING+ (propagates to run.log)
    """
    ...
    main.addHandler(_rotating(
        log_dir / "archived-products-negative.run.log",
    ...
    ok.propagate = False
    ok.handlers.clear()
    ok.addHandler(_rotating(log_dir / "archived-products-negative.success.log", logging.INFO))

    err = logging.getLogger("archived.err")
    err.setLevel(logging.WARNING)
    err.propagate = True
    err.handlers.clear()
    err.addHandler(_rotating(log_dir / "archived-products-negative.errors.log", logging.WARNING))
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="Concurrent batch + PUT workers (default 8)",
    )
    ap.add_argument(
        "--batch-size", type=int, default=50,
        help="Max products per batch call (default 50)",
    )
    ap.add_argument(
