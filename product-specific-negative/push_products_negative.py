...
Product-side counterpart of push_variants.py.

Reads `product_map.json` and the repaired product payloads produced by
`repair_products.py`, then pushes each non-archived product back to the
platform with a PUT, optionally followed by a batched COMMIT.

Filter
------
Only products in `product_map.json` where `state != ARCHIVED` are
processed. Archived products are skipped at scan time — those are
handled by repair_archived_products.py (a separate script with the
full recover→PUT→archive flow). The skipped-archived count is surfaced
in the run log.

Decision matrix (status from product_map.json):
...
Files read
----------
    <input>/product_map.json
    <input>/<tpl>/<siteIds-joined>/<productId>.json

Files written
-------------
    <input>/logs/push-products.run.log
    <input>/logs/push-products.success.log
    <input>/logs/push-products.errors.log
"""
...
# ---------------------------------------------------------------------------
# Site configuration — same as repair_products.py
# ---------------------------------------------------------------------------
...
def setup_logging(log_dir: Path, verbose: bool = False) -> logging.Logger:
    ...
    Three rotating handlers + console mirror at INFO+:
      ...
      push-products.run.log      — everything (DEBUG if --verbose else INFO)
      push-products.success.log  — per-product verdicts (INFO only)
      push-products.errors.log   — WARNING+ (propagates to run.log)
    """
    ...
    ok.propagate = False
    ok.handlers.clear()
    ok.addHandler(_rotating(log_dir / "push-products.success.log", logging.INFO))

    err = logging.getLogger("push.err")
    err.setLevel(logging.WARNING)
    err.propagate = True
    err.handlers.clear()
    err.addHandler(_rotating(log_dir / "push-products.errors.log", logging.WARNING))
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="Concurrent PUT workers (default 1)",
    )
    ap.add_argument(
